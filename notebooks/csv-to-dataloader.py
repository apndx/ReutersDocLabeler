...
def make_testloader(encodings, input_ids, labels, token_type_ids, attention_masks):
         # test to tensors
        t_test_inputs = torch.tensor(input_ids)
        t_test_labels = torch.tensor(labels)
        t_test_masks = torch.tensor(attention_masks)
        t_test_token_types = torch.tensor(token_type_ids)
        print('Data changed to tensors')
        print('test input:', t_test_inputs.shape)
        print('test labels:', t_test_labels.shape)
...
def main():
    ...
    # load csv
    df = pd.read_csv(f'notebooks/reuters-csv/{filename}', delimiter=';')
    print('Csv loaded')
    ...
    if sys.argv[2] == 'test':
        make_testloader(encodings, input_ids, labels, token_type_ids, attention_masks)
    else:
